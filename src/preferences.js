(() => {
  ...
  const TEXT = {
    zh: {
      ...
      importPersistenceNotice: "支持三种入口：① 单独或批量选择 JSON、MD、Markdown 文件；② 本工具导出的 ZIP 或文件夹；③ ChatGPT 官方导出文件夹（含 conversations 分片与 .dat 附件）。大型官方导出请先解压并使用“文件夹”导入，不建议直接导入数 GB 的 ZIP。导入内容仅供当前工作站临时读取，不会复制进浏览器永久储存，也不会修改电脑中的原文件。",
    ...
    en: {
      ...
      importPersistenceNotice: "Three import options are supported: (1) one or more JSON, MD, or Markdown files; (2) ZIP files or folders exported by this extension; and (3) official ChatGPT export folders containing conversation shards and .dat assets. Extract large official exports and use Folder import instead of selecting a multi-gigabyte ZIP. Imports are temporary, are not copied into permanent browser storage, and do not modify files on your computer.",
