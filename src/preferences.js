(() => {
  ...
  const TEXT = {
    zh: {
      ...
      importFolderTooltip: "导入备份文件夹",
      importPersistenceNotice: "ZIP 和文件夹仅供当前工作站页面临时读取，不会复制进浏览器永久储存；刷新或关闭页面后需要重新选择。电脑中的原文件不会被修改。",
      zipReadFailed: "ZIP 读取失败：{message}",
      zipNoSupportedFiles: "ZIP 中没有可读取的 JSON 或 Markdown 备份。",
    ...
    en: {
      ...
      importFolderTooltip: "Import a backup folder",
      importPersistenceNotice: "ZIP and folder imports are read only for the current workstation session. They are not copied into permanent browser storage, and must be selected again after the page is refreshed or closed. Files on your computer are not modified.",
      zipReadFailed: "Unable to read ZIP: {message}",
      zipNoSupportedFiles: "The ZIP does not contain readable JSON or Markdown backups.",
