...
> 跨平台实时备份  
> Cross-platform live backup

GPT 知识库是一个原创 Chrome Manifest V3 扩展，用于跨平台实时保存 AI 对话，并把用户选择的会话整理成后续生成 RAG 知识库的本地材料。

当前版本重点完成了 **跨平台实时备份、会话浏览、来源合并、选择与导出** 这一层。RAG 知识库生成会基于这些结构化会话继续扩展。
...
5. 将精选会话导出为 JSON / Markdown，作为 RAG 知识库的数据来源。

一句话定位：

> 跨平台实时备份 AI 会话，并将精选对话沉淀为可检索的 RAG 知识库材料。
...
- 文心一言 / Yiyan: `yiyan.baidu.com`, `wenxin.baidu.com`
...
- 完美级适配：ChatGPT、Claude、Gemini、Kimi。
- 可获取较全信息，但不能良好复原：豆包。
- 当前只能获取模型信息：DeepSeek。
...
- DeepSeek：当前只能获取模型信息，不标记为完整会话复原。
