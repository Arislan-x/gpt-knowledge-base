...
[简体中文](README.md) | [English](README_EN.md) | [隐私政策](PRIVACY.md)
...
- 支持全部本地会话合并导出 Markdown / JSON / HTML，或按会话导出 ZIP。
...
1. 在私有仓库的 [Releases](https://github.com/Arislan-x/gpt-knowledge-base/releases) 页面下载最新的 `gpt-knowledge-base-*.zip` 和 `.sha256` 文件。
...
私有商品介绍页见 [`product/index.html`](product/index.html)。该页面暂不公开部署。
...
本项目当前为私有专有软件。获得私有仓库或官方 Release 访问权限的用户仅获得有限的安装和授权使用权；除适用法律明确允许或获得书面许可外，不得复制、修改、再发布、转售或制作衍生产品。完整条款见 [`LICENSE`](LICENSE)。第三方组件和商标仍分别适用其自身许可或权利规则。
